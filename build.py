...
hbb_name = 'luoda' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: luoda
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: LUODA <info@dicad.cn>
Homepage: https://dicad.cn
...
def build_flutter_deb(version, features):
    ...
    system2('chmod +x tmpdeb/DEBIAN/preinst tmpdeb/DEBIAN/postinst tmpdeb/DEBIAN/prerm tmpdeb/DEBIAN/postrm')
...
def build_flutter_dmg(version, features):
    ...
    os.rename("rustdesk.dmg", f"../LUODA-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/luoda.exe')
    os.chdir('../..')
    if os.path.exists('./luoda_portable.exe'):
        ...
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './luoda_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/luoda_portable.exe')
    os.rename('./luoda_portable.exe', f'./LUODA-{version}.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/LUODA-{version}.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/luoda.exe target/release/LUODA.exe')
        pa = os.environ.get('P')
        if pa:
        ...
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/LUODA-{version}-win7-install.exe')
    ...
    '''.format(pa, version))
                ...
                os.system('chmod +x tmpdeb/DEBIAN/preinst tmpdeb/DEBIAN/postinst tmpdeb/DEBIAN/prerm tmpdeb/DEBIAN/postrm')
                ...
                os.rename('rustdesk.deb', 'LUODA-%s.deb' % version)
